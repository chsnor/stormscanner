...
async def scan_live_pair(name, yf_ticker, tf_base, tf_htf, winrate):
    ...
    try:
        ...
        # คืน Event Loop ชั่วคราวก่อนคำนวณหนัก
        await asyncio.sleep(0)

        # 2) คำนวณสัญญาณ Storm A+ (CPU-heavy → รันใน thread)
        ...
        thai_time_str = str(last_bar_time + pd.Timedelta(hours=7))

        new_trade = {
            'trade_id': trade_id, 'timestamp': thai_time_str,
...
async def scanner_worker():
    ...
    while True:
        # สแกนทีละคู่แบบต่อเนื่อง + ปล่อย Event Loop ระหว่างคู่
        # เพื่อให้ Web Server ตอบ Health Check ได้ตลอดเวลา
        for name, ticker, tf_b, tf_h, wr in A_PLUS_WATCHLIST:
            await scan_live_pair(name, ticker, tf_b, tf_h, wr)
            await asyncio.sleep(0.5)  # หยุด 0.5 วินาทีระหว่างคู่
