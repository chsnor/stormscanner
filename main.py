...
def calc_percentile_rank(series, length):
    ...
    for i in range(1, n):
        ...
        valid_count = np.count_nonzero(~np.isnan(window))
...
def compute_storm_signals(data_b, data_h):
    ...
    bs_touch_up = 999; bs_touch_dn = 999
    ...
    for i in range(n):
        ...
        sEmaL = (trendUp and bs_touch_up <= 3 and not np.isnan(ema9[i])
                 and close[i] > ema9[i] and close[i] > open_[i] and close[i] > prevH)
        sEmaS = (trendDn and bs_touch_dn <= 3 and not np.isnan(ema9[i])
                 and close[i] < ema9[i] and close[i] < open_[i] and close[i] < prevL)
        ...
        if sBrtL: brt_hi_brk = False
        if sBrtS: brt_lo_brk = False
        sSweepL = sSweepS = False
        if i >= 22:
            swLo = np.min(low[i - 21:i - 1])
            swHi = np.max(high[i - 21:i - 1])
        ...
        if AUTO_RSI:
            if s_on.total >= 5 and s_off.total >= 5:
                if wr_off > wr_on: prefer_on = False
                elif wr_off == wr_on and s_off.total > s_on.total: prefer_on = False
            elif s_off.total >= 5 and s_on.total < 5:
                if wr_off >= 60.0: prefer_on = False
            elif s_on.total >= 5 and s_off.total < 5:
                if wr_on < 40.0: prefer_on = False
        ...
        sig_out = {
            'signal': sig, 'setups': setups if sig else [],
            'regime': regime, 'vol_pct': round(vp, 1), 'rsi': round(rv, 1),
            'atr': ai, 'prefer_on': prefer_on,
            'wr_on': round(wr_on, 1), 'wr_off': round(wr_off, 1),
            'trades_on': s_on.total, 'trades_off': s_off.total,
    ...
    for t in active_trades:
        ...
        for bar_count, (idx, bar) in enumerate(new_bars.iterrows()):
            ...
            if hit_stop and hit_tp2:
                ...
                if sl_first:
                    tp1w = tp1_first_heuristic(t.get('hit_tp1', False), direction, t['tp1'], hit_tp1, c, o)
                ...
                em = "🚀" if pnl > 0 else "🛑"
                await send_telegram(f"{em} *[{status}]* `{t['pair']}` *[{t['tf']}]* ({t['side']}) | PnL: {pnl:+.1f}R")
                ...
                await send_telegram(f"🚀 *[TP2 FULL WIN]* `{t['pair']}` *[{t['tf']}]* ({t['side']}) | +2.0R ✅")
                ...
                update_csv_trade_status(t['trade_id'], status, pnl, now_str)
                await send_telegram(f"⏰ *[{status}]* `{t['pair']}` *[{t['tf']}]* ({t['side']}) | PnL: {pnl:+.1f}R")
                ...
                if hit_tp1 and not t.get('hit_tp1', False):
                    ...
                    await send_telegram(f"🎯 *[TP1 HIT]* `{t['pair']}` *[{t['tf']}]* ({t['side']}) | +0.8R ✨")
...
async def scan_live_pair(name, yf_ticker, tf_base, tf_htf, winrate):
    ...
    try:
        ...
        # ดึงข้อมูลใน thread ต่างหากไม่บล็อก event loop
        ...
        # คืน event loop ก่อนการคำนวณหนัก
        await asyncio.sleep(0)
        # คำนวณสัญญาณใน thread ต่างหากไม่บล็อก web server
        result = await asyncio.to_thread(compute_storm_signals, confirmed, data_h)
        ...
        new_trade = {
            ...
            'open_bar_time': last_time, 'hit_tp1': False, 'last_checked_bar': last_time,
        }
    ...
    <head>
        ...
        <style>
            body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
                   background: #0b0f19; color: #f1f5f9; margin: 0; padding: 20px; }}
            .container {{ max-width: 1200px; margin: 0 auto; }}
            .header {{ display: flex; justify-content: space-between; align-items: center;
                      background: #1e293b; padding: 20px 25px; border-radius: 12px;
                      border: 1px solid #334155; margin-bottom: 20px; }}
            .title {{ font-size: 22px; font-weight: bold; color: #38bdf8;
                     display: flex; align-items: center; gap: 10px; }}
            .badge-live {{ background: #10b981; color: white; padding: 4px 12px;
                          border-radius: 20px; font-size: 13px; font-weight: bold; }}
            .card {{ background: #1e293b; padding: 20px; border-radius: 12px;
                    border: 1px solid #334155; margin-bottom: 20px; }}
            .card-title {{ font-size: 17px; font-weight: bold; margin-bottom: 12px; color: #94a3b8; }}
    ...
    <body>
        <div class="container">
            ...
            <div class="card">
                ...
                <div class="log-terminal">{logs_text if logs_text else 'ระบบกำลังเฝ้ากราฟ A+ pairs...'}</div>
            </div>
            <div class="card">
                ...
                <table>
                    ...
                    <tbody>
                        {rows_html if rows_html else
                         '<tr><td colspan="9" style="text-align:center; color:#64748b;">ยังไม่มีสัญญาณ</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
async def start_web_server():
    ...
    await site.start()
    log_event(f"Web Server listening on port {port}")
...
async def scanner_worker():
    ...
    while True:
        # สแกนทีละคู่แบบ Sequential + ปล่อย Event Loop
        # ทำให้ Web Server ตอบ Health Check ได้ตลอดเวลา
        for name, ticker, tf_b, tf_h, wr in A_PLUS_WATCHLIST:
            await scan_live_pair(name, ticker, tf_b, tf_h, wr)
            await asyncio.sleep(0.5)  # หยุดปล่อย Event Loop ระหว่างคู่
